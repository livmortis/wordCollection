package com.example.xzy.wordidtyandcltn.view;
...
public class AreaView extends View {
	...
	public static final int	EDIT	= 0;				//创建遮挡区域
	public static final int	DELETE	= 1;				//清空遮挡区域
	public static final int	DRAG	= 2;
	
	private static final String NAMESPACE = "http://schemas.android.com/apk/res/android";

	private int			mChooseColor	= Color.rgb(255, 0, 0);
	private int			mMoveColor		= Color.rgb(0, 161, 233);
	private int 		mLineColor1 	= Color.rgb(177, 177, 148);
	private int 		mLineColor2 	= Color.rgb(255, 255, 255);
	private int			mCellAlpha		= 70;
...
//	}
	...
	public void setAreas(byte[][] region)
	{
	...
	public void clearAerea(byte[][] region)
	{
		for (int i = 0; i < region.length; i++)
		{
			for (int j = 0; j < region[i].length; j++)
...
//				if (region[i][j] != 0)
//				{
				mCellList.get(i * mCol + j).choose = false;
//				}
			}
		}
...
//				}
	...
	public boolean anyAerea(byte[][] region)
	{
		for (int i = 0; i < region.length; i++)
		{
			for (int j = 0; j < region[i].length; j++)
...
//				}
			...
			case DRAG:
				if(ZoomStartdis != 0  && event.getPointerCount() > 1)
				{
					...
					iviv.setLayoutParams(new FrameLayout.LayoutParams(width, height));
					((FrameLayout)this.getParent()).setLayoutParams(new FrameLayout.LayoutParams(width, height));
				}
				break;
			default:
				break;
 		...
 		switch(mMode)
		{
			case EDIT:
		 		...
		 		for(Cell cell: mCellList)
				{
		 		...
		 		invalidate();
//				FlurryUtility.logEvent(getContext(), AppConstant.FlurryKey.DEV_SETTING_DRAG_REGION);
				break;
			case DELETE:
